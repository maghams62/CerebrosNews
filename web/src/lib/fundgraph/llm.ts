...
async function loadPrompt(
  ...
  try {
    ...
    if (name === "generate_fund_memo") {
      return [
        "You are writing a coherent investment decision memo for one fund.",
        "Use only the provided packet. Never invent details.",
        "Return JSON only with shape: { sections: [{ key, title, content }] }.",
        "Include sections in this order:",
        "executive_summary, fund_overview, team_gp_assessment, strategy, portfolio_snapshot, key_signals_recent_activity, network_position, graph_snapshot, graph_analyzer_insights, bull_case, risks_concerns, open_questions, final_view.",
        "Write each section as: one orienting line + 3-7 concise bullets.",
        "Avoid repeating the same facts across sections; add interpretation, not duplication.",
        "For factual statements from claims/signals, include citation tags like [S3] or [C2] when available.",
        "If missing, explicitly state Unknown or Data gap.",
        "Final view must include: Recommendation (Pursue/Monitor/Pass), Confidence (High/Medium/Low), Next action.",
        "Subject: {{subject}}",
        "Options: {{options}}",
        "Packet: {{packet}}",
      ].join("\n");
    ...
    if (name === "generate_watchlist_brief") {
      return [
        "You are writing a coherent watchlist brief across selected funds.",
        "Use only the provided packet. Do not invent facts.",
        "Do not rank funds or imply winners/losers.",
        "Return JSON only with shape: { sections: [{ key, title, content }] }.",
        "Include sections in this order:",
        "watchlist_snapshot, today_highlights, watchlist_ranking, watchlist_key_signals, graph_snapshot, cross_fund_network_signals, watchlist_risks, watchlist_next_steps.",
        "Keep key watchlist_ranking for compatibility, but write neutral per-fund research notes (not ranking).",
        "Write each section as: one orienting line + 3-7 concise bullets.",
        "Explicitly compare shared vs fund-specific patterns in portfolio/signal evidence.",
