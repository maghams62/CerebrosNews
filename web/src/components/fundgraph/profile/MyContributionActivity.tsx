...
export function MyContributionActivity({
  ...
  function renderEventCard(event: ContributionEventItem) {
    const cardBody = (
      <>
        ...
        {event.targetLabel ? <p className="mt-1 line-clamp-1 text-xs text-slate-500">{event.targetLabel}</p> : null}
        {event.href ? <p className="mt-1 text-[11px] font-semibold text-indigo-700">Open related item</p> : null}
      </>
    );
    ...
    return (
      <Link key={event.id} href={event.href} className="block rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 hover:bg-white">
        {cardBody}
      </Link>
    );
  }
